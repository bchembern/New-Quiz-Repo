...
const cardColors = ['#FFA07A', '#FA8072', '#E9967A', '#F08080', '#CD5C5C'];
...
function loadAnswers() {
  ...
  answers.forEach((text, index) => {
      ...
      const randomColor = `hsl(${Math.random() * 360}, 100%, 75%)`; // Generates a random hue with full saturation and lightness at 75%
      card.style.backgroundColor = randomColor;

      answerDiv.appendChild(card);
  });

  // Ensure the first answer is positioned correctly
  currentCardIndex = 0;
  updateCardsPosition(); // Call this function to update the positions after loading new answers
}
